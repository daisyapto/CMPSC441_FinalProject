...
# MANUAL IMAGE PREDICTION
def predict_image(model_type, file_path):
    if not file_path:
        return None
...
def predict(model_type, test_type, max_images=None, file_path=None):
    if test_type == "manual":
        return predict_image(model_type, file_path)
    else:
        return predict_database(model_type, max_images)
